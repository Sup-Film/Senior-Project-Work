...
fetch('/projectsenior/classroom', {
  ...
  .then(data => {
    ...
    // console.log(data);
    data.forEach(each => {
      const encodedId = btoa(each._id); // encode id with base64
      ...
      <div class="gallery gallery--grid">
        <div class="gallery__item">
          <div class="cards">
            ...
            <a href="deck?classroom=${encodedId}" class="button button--primary trade" type="button">Go</a>
            </button>
          </div>
        </div>
      </div>
...
formupdate.addEventListener("submit", (e) => {
  e.preventDefault();
  const updateName = document.getElementById("updatenameroom").value;
  const namerooms = updateName.toUpperCase();
