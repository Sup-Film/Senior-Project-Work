...
deckRouter.delete('/flashcard/:id', (req, res, next) => {
  ...
  Flashcard.findByIdAndRemove(flashcardId, req.body)
    .then(() => {
      const deleteQuiz = Quiz.deleteMany({ flashcard_id: flashcardId });
      const deleteFlashcard = Deck.updateMany({ flashcards: flashcardId }, { $pull: { flashcards: flashcardId } })
      Promise.all([deleteQuiz, deleteFlashcard])
        .then(() => {
          res.json({ message: 'Deleted' });
        })
        .catch((err) => {
...
deckRouter.put('/quiz/update/:id', (req, res, next) => {
  ...
  const quizQuestion = req.body.quiz_question;
  const deck_id = req.body.deck_id;
  console.log(req.body)
  // console.log(deck_id)
  Flashcard.findOne({ card_question: quizQuestion, deck_id: deck_id})
    .then((flashcard) => {
      if (!flashcard) {
        res.json({ message: 'Flashcard not found' });
      }
      return Quiz.findByIdAndUpdate(
        quizId,
        { ...req.body, flashcard_id: flashcard._id },
        { new: true }
      );
    })
    .then(() => {
...
deckRouter.delete('/quiz/del/:id', (req, res, next) => {
  Quiz.findByIdAndRemove(req.params.id, req.body)
    .then(() => {
      Deck.updateMany({ quizzes: req.params.id }, { $pull: { quizzes: req.params.id } })
        .then(() => {
          res.json({ message: 'Deleted' });
        })
        .catch((err) => {
...
deckRouter.get('/compareflashcardAndQuiz/:deck_id', async (req, res, next) => {
  try {
    ...
    if ((!flashcards || flashcards.length === 0) && (!quizzes || quizzes.length === 0)) {
      res.status(404).json({ message: 'กรุณาทำการเพิ่ม Flashcard และ Quiz ก่อนทำการเรียน' });
